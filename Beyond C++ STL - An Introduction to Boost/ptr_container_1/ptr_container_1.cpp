...
//
// Define a small polymorphic class hierarchy
//
...
class animal : boost::noncopyable {
	...
	std::string name_;
protected:
	// this class (animal) can not be copied...
	animal(const animal& r) : name_(r.name_) {}
	void operator=(const animal& r);
private:
...
//
// class 'animal' is still not cloneable; we need this last hook;
// notice that we pass the animal by const ref and return a pointer
//
...
public:
	// make it possible to iterate over the animals in the farm
    ...
    //
	farm(animal_iterator begin, animal_iterator end) : barn(begin, end) {}
...
int main(int argc, char** argv) {
	...
	//
    // Are there really clones in the new farm?
    //
    BOOST_ASSERT( new_farm.begin()->name() == "Betty" );

	//
    // Then we search for an animal, Mary, 
